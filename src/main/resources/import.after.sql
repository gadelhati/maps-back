...
INSERT INTO maps.upper_air (created_at, updated_at, id, cod_estacao, alt, dbc, pressao, dd, fff, ido, temp, ur, hora, ascensao, orvalho, ctrlqc_pressao, ctrlqc_vento, ctrlqc_temp, ctrlqc_orvalho, ctrlqc_ur, ctrlqc_alt, ctrlqc_dd, ctrlqc_fff) SELECT NOW(), NOW(), gen_random_uuid(), t2.cod_estacao, t2.alt, TRIM(t2.dbc)::INTEGER, t2.pressao, t2.dd, t2.fff, t2.ido::INTEGER, t2.temp, t2.ur,
	CASE WHEN SUBSTRING(t2.hora, 4, 1)::INTEGER > 5 THEN CONCAT(SUBSTRING(t2.hora, 1, 1), SUBSTRING(t2.hora, 2, 1)::INTEGER + (SUBSTRING(t2.hora, 4,1)::INTEGER - 5), ':', (SUBSTRING(t2.hora, 4,1)::INTEGER - 5), SUBSTRING(t2.hora, 5, 4))::time
	ELSE t2.hora::time
	END hora_corrigida,
	t2.ascensao, t2.orvalho, t2.ctrlqc_pressao::INTEGER, t2.ctrlqc_vento::INTEGER, t2.ctrlqc_temp::INTEGER, t2.ctrlqc_orvalho::INTEGER, t2.ctrlqc_ur::INTEGER, t2.ctrlqc_alt::INTEGER, t2.ctrlqc_dd::INTEGER, t2.ctrlqc_fff::INTEGER FROM sisbndo.tb_ar_superior AS t2 ON CONFLICT DO NOTHING;


INSERT INTO maps.eet (created_at, updated_at, id, code, station_category, commission, platform, equipment, media, start, finish, latitude_bottom_most, latitude_top_most, longitude_right_most, longitude_left_most, fb, plataforma_batimetrica, secrecy_code, obs, equipment_depth, local_depth, datum, name_seismic_line, start_seismic_line_data, end_seismic_line_data)
    SELECT NOW(), NOW(), gen_random_uuid(), t2.cod_estacao_espaco_tempo, (SELECT t3.id FROM maps.station_category AS t3 WHERE t3.code = t2.cod_tipo_estacao), (SELECT t3.id FROM maps.commission AS t3 WHERE t3.code = t2.cod_comissao), (SELECT t3.id FROM maps.platform AS t3 WHERE t3.code = t2.cod_plataforma), (SELECT t3.id FROM maps.equipment AS t3 WHERE t3.code = t2.cod_equipamento), (SELECT t3.id FROM maps.media AS t3 WHERE t3.code = t2.cod_midia), t2.data_hora_inicio, t2.data_hora_fim, t2.lat_bottommost, t2.lat_topmost, t2.long_rightmost, t2.long_leftmost, t2.fb, t2.plataforma_batimetrica, t2.cod_sigilo, t2.obs, t2.prof_eqpto, t2.prof_local, (SELECT t3.id FROM maps.datum AS t3 WHERE t3.code = t2.cod_datum), t2.nome_linha_sism, t2.data_ini_linha_sism, t2.data_fim_linha_sism  FROM sisbndo.tb_estacao_espaco_tempo AS t2 ON CONFLICT DO NOTHING;
...
UPDATE maps.bathymetry SET point = 
	CONCAT('POINT(',
	CASE 
		WHEN SUBSTRING(longitude::VARCHAR, 1, 1) = '-' AND LENGTH(REPLACE(longitude::VARCHAR, '-', '')) > 2
		   THEN CONCAT('-', SUBSTRING(REPLACE(longitude::VARCHAR, '-', ''), 1, 2), '.', SUBSTRING(REPLACE(longitude::VARCHAR, '-', ''), 3))::NUMERIC
		WHEN SUBSTRING(longitude::VARCHAR, 1, 1) != '-' AND LENGTH(REPLACE(longitude::VARCHAR, '-', '')) > 2
		   THEN CONCAT(SUBSTRING(longitude::VARCHAR, 1, 2), '.', SUBSTRING(longitude::VARCHAR, 3))::NUMERIC
		ELSE longitude::NUMERIC
		END,
	' ',
	CASE
		WHEN SUBSTRING(latitude::VARCHAR, 1, 1) = '-' AND LENGTH(REPLACE(latitude::VARCHAR, '-', '')) > 2 AND SUBSTRING(latitude::VARCHAR, 1, 3)::NUMERIC > -90
		   THEN CONCAT('-', SUBSTRING(REPLACE(latitude::VARCHAR, '-', ''), 1, 2), '.', SUBSTRING(REPLACE(latitude::VARCHAR, '-', ''), 3))::NUMERIC
		WHEN SUBSTRING(latitude::VARCHAR, 1, 1) != '-' AND LENGTH(REPLACE(latitude::VARCHAR, '-', '')) > 2 AND SUBSTRING(latitude::VARCHAR, 1, 2)::NUMERIC < 90
		   THEN CONCAT(SUBSTRING(latitude::VARCHAR, 1, 2), '.', SUBSTRING(latitude::VARCHAR, 3))::NUMERIC
		WHEN SUBSTRING(latitude::VARCHAR, 1, 1) = '-' AND LENGTH(REPLACE(latitude::VARCHAR, '-', '')) >= 2 AND SUBSTRING(latitude::VARCHAR, 1, 3)::NUMERIC <= -90
		   THEN CONCAT('-', SUBSTRING(REPLACE(latitude::VARCHAR, '-', ''), 1, 1), '.', SUBSTRING(REPLACE(latitude::VARCHAR, '-', ''), 2))::NUMERIC
		WHEN SUBSTRING(latitude::VARCHAR, 1, 1) != '-' AND LENGTH(REPLACE(latitude::VARCHAR, '-', '')) >= 2 AND SUBSTRING(latitude::VARCHAR, 1, 2)::NUMERIC >= 90
		   THEN CONCAT(SUBSTRING(latitude::VARCHAR, 1, 1), '.', SUBSTRING(latitude::VARCHAR, 2))::NUMERIC
