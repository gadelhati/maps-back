...
public class ControllerGaugeStation implements ControllerInterface<DTOResponseGaugeStation, DTORequestGaugeStation> {
    ...
    public ResponseEntity<Page<DTOResponseGaugeStation>> retrieve(@RequestParam(name = "key", defaultValue = "", required = false) String key, @RequestParam(name="value", defaultValue = "", required = false) String value, Pageable pageable){
        return ResponseEntity.ok().body(serviceGaugeStation.retrieve(pageable, key, value));
    }
